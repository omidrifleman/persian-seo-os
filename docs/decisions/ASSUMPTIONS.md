...
**برچسب:** `NEEDS-CONFIRMATION`
آداپتورهای داخل `fa`/`persian.kasreh.mdl` با نام تسک `ner`
...
رسیده، نه وزن‌های یک مدل NER بی‌ربط.
**این فرض هنوز اثبات نشده.** قطعیت استنتاج (seed) با shim برقرار شد؛ درستی
برچسب‌ها روی دادهٔ طلایی هنوز باز است.
**شرط بسته‌شدن:** (۱) precision/recall/F1 روی مجموعهٔ برچسب‌خوردهٔ کسره با
حداقل چند صد توکن، یا (۲) تأیید صریح نویسندگان DadmaTools که کلیدهای `ner`
داخل `kasreh.mdl` همان آداپتور kasrehاند.
