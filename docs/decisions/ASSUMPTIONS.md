...
**برچسب:** `CONFIRMED` (نام‌گذاری + حذف ابزار دروغین؛ منبع داده هنوز باز)
دادهٔ حجم جست‌وجوی فارسی اغلب تخمینی یا ناموجود است. فیلدها فقط
`search_demand` + `search_demand_status` با حالت‌های
`known` / `estimated` / `unknown` — هرگز عدد ساختگی.
ابزار MCP `keywords.volume_lookup` از `TOOLS` حذف شد: بدون منبع واقعی،
حضور در فهرست ابزار یک ادعای دروغ است (ADR-0010).
**شرط بازگشت ابزار lookup:** اتصال به یک منبع دادهٔ واقعی (API/کاشهٔ
مجوزدار)، نه rename یا stub که همیشه `unknown` برگرداند.
